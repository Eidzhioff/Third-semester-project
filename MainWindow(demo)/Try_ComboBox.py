...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.comboBox.addItem("po vozrastaniy")
        self.comboBox.addItem("po ybivaniy")
        self.comboBox.currentIndexChanged.connect(self.changeComboBox)
        self.content = "po vozrastaniy"
    ...
    #Проверка выброного мода
    def changeComboBox(self, index):
        self.content = self.comboBox.itemText(index)
    ...
    def export_res(self):
        ...
        #Подключение к Бд
        #Вывод в зависимоти от мода ComboBox'a
        if self.content == "po vozrastaniy":
            
            with sq.connect("newdb.db") as con:
                cur = con.cursor()
                
                l = "SELECT new_apart, cost_apart FROM apart WHERE " + aa + " AND" + price + "ORDER BY cost_apart" #sql запрос  
                cur.execute(f"{l}")
        ...
        elif self.content == "po ybivaniy":
            
            with sq.connect("newdb.db") as con:
                cur = con.cursor()
                
                l = "SELECT new_apart, cost_apart FROM apart WHERE " + aa + " AND" + price + "ORDER BY cost_apart DESC" #sql запрос  
                cur.execute(f"{l}")
